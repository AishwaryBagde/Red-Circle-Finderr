...
import demo.RedCircleDetector;


public class RedCircleFinder {
    private WebDriver driver;

    public RedCircleFinder(WebDriver driver) {
        this.driver = driver;
    }
    ...
    public void downloadRedImages() throws IOException, InterruptedException {
        ...
        for (WebElement image : images) {
            ...
            if (src != null && src.startsWith("http")) { // Check if URL is valid
                try {
                    String filePath = "red_images/image_" + count + ".jpg";
                    System.out.println("Downloading: " + src);
                    downloadImage(src, filePath);

                    // Check if the downloaded image contains a red circle
                    if (RedCircleDetector.isRedCircle(filePath)) {
                        System.out.println("Red circle detected in: " + filePath);
                    } else {
                        System.out.println("No red circle detected in: " + filePath);
                        // Optionally, delete the image if no red circle is found
                    ...
                    System.err.println("Failed to download or process image: " + src + " - " + e.getMessage());
                }
            } else {
